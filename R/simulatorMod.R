...
calculatorServer <- function(input, output, session, userInputs){
  ...
  #Perform simulation in python after the user clicks the Calculate button.
	...
	output$troubleshootingtext <- shiny::renderUI({shiny::HTML(ifelse(userInputs$issues == 1,
	...
	output$plot1header <- shiny::renderUI({shiny::h3("Calculate Theoretical Duplicates")})

	output$plot3header <- shiny::renderUI({shiny::h3("Calculate Mean Clone Size in the Entire Cell Population Before Sequencing")})

	output$plot4header <- shiny::renderUI({shiny::h3("Calculate Sequenced Clone Size Distribution After a Single Experiment")})
	...
	output$sidebar <- shiny::renderUI({
	  ...
	  switch(
	    ...
	    "five" = shiny::selectInput("issues", shiny::h4("Select one"), choices = list("Where do I start?" = 1,
